...
import './index.css';
import App from './App';
import { trigger, query, listen, filter, after } from 'polyrhythm';
...
// Create a derived Observable that is debounced..
// query('search/change').pipe(
//   _filter(event => (event.payload.q || '').length > 2),
//   debounceTime(700),
//   tap(({ payload: q }) => trigger('search/start', { q }))
// ).subscribe();

// OR equivalently replace a series of (debounce time + ajax)
listen(
  'search/change',
  ({ payload: { q } }) => {
